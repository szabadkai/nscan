...
# Suppress progress bars to prevent flickering
...
# Use runspaces for truly silent parallel execution
...
foreach ($i in 1..254) {
    ...
    }).AddArgument($baseIP).AddArgument($i)
...
if ($activeHosts.Count -gt 0) {
    ...
    Write-Host "  Scanning common ports..." -ForegroundColor Gray
    ...
    foreach ($result in $portResults) {
        ...
        if ($openPorts.Count -gt 0) {
            ...
            Write-Host " -> " -NoNewline -ForegroundColor DarkGray
            Write-Host "$($portsStr.PadRight(27))" -NoNewline -ForegroundColor Green
            Write-Host "|" -ForegroundColor DarkGray
        } else {
            $namePart = $displayName.PadRight(25)
            Write-Host "|  " -NoNewline -ForegroundColor DarkGray
